...
describe("sync routes", () => {
  ...
  it("rejects repeated same-entity updates with a stale intra-batch base revision without mutation", async () => {
    const now = "2026-01-01T00:00:00.000Z";
    await baseEnv.DB.batch([
      baseEnv.DB.prepare(
        `INSERT INTO folders (
    ...
    const response = await pushSync(
      {
        ...
        changes: [
          {
            clientChangeId: "chg_note_update_first",
            entityType: "note",
            entityId: "note_1",
            operation: "update",
            baseRevision: 1,
          ...
          {
            clientChangeId: "chg_note_update_second_stale",
            entityType: "note",
            entityId: "note_1",
            operation: "update",
            baseRevision: 1,
    ...
    const note = await baseEnv.DB.prepare(
      `SELECT title, body_md AS bodyMd, current_revision AS currentRevision
       FROM notes
       WHERE id = ? AND user_id = ?`,
    )
      .bind("note_1", "user_dev")
      .first<{ title: string; bodyMd: string; currentRevision: number }>();
    ...
    const eventCount = await baseEnv.DB.prepare(
      ...
      .bind("user_dev", "note_1")
      .first<{ count: number }>();

    expect(eventCount?.count).toBe(0);
  });

  it("rejects duplicate client change ids in one push without mutation", async () => {
    const response = await pushSync(
      {
        deviceId: "web_1",
        changes: [
          {
            clientChangeId: "chg_duplicate_in_batch",
            entityType: "folder",
            entityId: "folder_1",
            operation: "create",
            baseRevision: 0,
    ...
    const folderCount = await baseEnv.DB.prepare(
      `SELECT COUNT(*) AS count
       FROM folders
       WHERE user_id = ? AND id IN (?, ?)`,
    )
      .bind("user_dev", "folder_1", "folder_2")
      .first<{ count: number }>();

    expect(folderCount?.count).toBe(0);
    ...
    const eventCount = await baseEnv.DB.prepare(
      ...
      .bind("user_dev", "chg_duplicate_in_batch")
      .first<{ count: number }>();
  ...
  it("rejects repeated same-entity note deletes with a stale intra-batch base revision without mutation", async () => {
    const now = "2026-01-01T00:00:00.000Z";
    await baseEnv.DB.batch([
      baseEnv.DB.prepare(
        `INSERT INTO folders (
      ...
      ).bind("note_1", "user_dev", "folder_1", "Active", "Body", "Body", 1, now, now),
    ]);
    ...
    const response = await pushSync(
      {
        ...
        changes: [
          {
            clientChangeId: "chg_note_delete_first",
            entityType: "note",
            entityId: "note_1",
            operation: "delete",
            baseRevision: 1,
          ...
          {
            clientChangeId: "chg_note_delete_second_stale",
            entityType: "note",
            entityId: "note_1",
            operation: "delete",
            baseRevision: 1,
    ...
    const eventCount = await baseEnv.DB.prepare(
      ...
      .bind("user_dev", "chg_note_delete_first", "chg_note_delete_second_stale")
      .first<{ count: number }>();
  ...
  it("rejects repeated same-entity folder deletes with a stale intra-batch base revision without mutation", async () => {
    const now = "2026-01-01T00:00:00.000Z";
    await baseEnv.DB.batch([
      baseEnv.DB.prepare(
        `INSERT INTO folders (
    ...
    const response = await pushSync(
      {
        ...
        changes: [
          {
            clientChangeId: "chg_folder_delete_first",
            entityType: "folder",
            entityId: "folder_1",
            operation: "delete",
            baseRevision: 1,
          ...
          {
            clientChangeId: "chg_folder_delete_second_stale",
            entityType: "folder",
            entityId: "folder_1",
            operation: "delete",
            baseRevision: 1,
    ...
    const eventCount = await baseEnv.DB.prepare(
      ...
      .bind(
        "user_dev",
        "chg_folder_delete_first",
        "chg_folder_delete_second_stale",
        "cascade_folder_1_note_1",
      )
      .first<{ count: number }>();
  ...
  it("rejects folder delete before a later note create in the deleted folder without partial mutation", async () => {
    const now = "2026-01-01T00:00:00.000Z";
    await baseEnv.DB.prepare(
      `INSERT INTO folders (
        id, user_id, name, sort_order, current_revision, created_at, updated_at, deleted_at
    ...
    const response = await pushSync(
      {
        ...
        changes: [
          {
            clientChangeId: "chg_folder_delete_before_note_create",
            entityType: "folder",
            entityId: "folder_1",
            operation: "delete",
            baseRevision: 1,
          ...
          {
            clientChangeId: "chg_note_create_after_folder_delete",
            entityType: "note",
            entityId: "note_1",
            operation: "create",
            baseRevision: 0,
    ...
    const noteCount = await baseEnv.DB.prepare(
      `SELECT COUNT(*) AS count
       FROM notes
    ...
    const eventCount = await baseEnv.DB.prepare(
      ...
      .bind(
        "user_dev",
        "chg_folder_delete_before_note_create",
        "chg_note_create_after_folder_delete",
      )
      .first<{ count: number }>();
  ...
  it("rejects folder delete before a later note move into the deleted folder without partial mutation", async () => {
    const now = "2026-01-01T00:00:00.000Z";
    await baseEnv.DB.batch([
      baseEnv.DB.prepare(
        `INSERT INTO folders (
      ...
      ).bind("folder_2", "user_dev", "Archive", 2, 1, now, now),
      baseEnv.DB.prepare(
        `INSERT INTO notes (
          id, user_id, folder_id, title, body_md, body_plain, current_revision, created_at, updated_at, deleted_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)`,
      ).bind("note_1", "user_dev", "folder_2", "Original", "Original Body", "Original Body", 1, now, now),
    ]);
    ...
    const response = await pushSync(
      {
        ...
        changes: [
          {
            clientChangeId: "chg_folder_delete_before_note_move",
            entityType: "folder",
            entityId: "folder_1",
            operation: "delete",
            baseRevision: 1,
          ...
          {
            clientChangeId: "chg_note_move_after_folder_delete",
            entityType: "note",
            entityId: "note_1",
            operation: "update",
            baseRevision: 1,
    ...
    const note = await baseEnv.DB.prepare(
      `SELECT folder_id AS folderId, title, body_md AS bodyMd, current_revision AS currentRevision
       FROM notes
       WHERE id = ? AND user_id = ?`,
    )
      .bind("note_1", "user_dev")
      .first<{ folderId: string; title: string; bodyMd: string; currentRevision: number }>();
    ...
    const eventCount = await baseEnv.DB.prepare(
      ...
      .bind(
        "user_dev",
        "chg_folder_delete_before_note_move",
        "chg_note_move_after_folder_delete",
      )
      .first<{ count: number }>();
  ...
  it("returns the max cursor when the last accepted change is an older idempotent replay", async () => {
    const now = "2026-01-01T00:00:00.000Z";
    await baseEnv.DB.batch([
      baseEnv.DB.prepare(
        `INSERT INTO folders (
      ...
      ).bind("note_1", "user_dev", "folder_1", "Existing", "Body", "Body", 2, now, now),
      baseEnv.DB.prepare(
        `INSERT INTO sync_events (
          id, user_id, entity_type, entity_id, operation,
          revision_number, client_change_id, source_device_id, created_at
        ...
        "evt_existing_note_update_cursor",
        "user_dev",
        "note",
        "note_1",
        "update",
        2,
        "chg_existing_note_update_cursor",
        "web_1",
        now,
      ),
    ]);
    ...
    const response = await pushSync(
      {
        ...
        changes: [
          {
            clientChangeId: "chg_folder_update_before_replay",
            entityType: "folder",
            entityId: "folder_1",
            operation: "update",
            baseRevision: 1,
          ...
          {
            clientChangeId: "chg_existing_note_update_cursor",
            entityType: "note",
            entityId: "note_1",
            operation: "update",
            baseRevision: 1,
  ...
  it("repairs missing folder delete cascade work on idempotent retry", async () => {
    const now = "2026-01-01T00:00:00.000Z";
    await baseEnv.DB.batch([
      baseEnv.DB.prepare(
        `INSERT INTO folders (
      ...
      ).bind("folder_1", "user_dev", "Inbox", 1, 2, now, now, now),
      baseEnv.DB.prepare(
        `INSERT INTO notes (
          id, user_id, folder_id, title, body_md, body_plain, current_revision, created_at, updated_at, deleted_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)`,
      ).bind("note_1", "user_dev", "folder_1", "Stranded", "Body", "Body", 1, now, now),
      baseEnv.DB.prepare(
        `INSERT INTO sync_events (
          id, user_id, entity_type, entity_id, operation,
          revision_number, client_change_id, source_device_id, created_at
        ...
        "evt_partial_folder_delete",
        "user_dev",
        "folder",
        "folder_1",
        "delete",
        2,
        "chg_folder_delete_partial_retry",
        "web_1",
        now,
      ),
    ]);
    ...
    const response = await pushSync(
      {
        ...
        changes: [
          {
            clientChangeId: "chg_folder_delete_partial_retry",
            entityType: "folder",
            entityId: "folder_1",
            operation: "delete",
            baseRevision: 1,
